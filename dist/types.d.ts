...
export type IRaceIcons = Record<string, [
    x: number,
    y: number,
    width: number,
    height: number
...
export type GetObjectFunction = <T extends keyof ObjectsTypeMap>(key: T, id: string) => ObjectsTypeMap[T] | undefined;
...
export type ChangeTuple<T> = [
    'replace' | 'change',
    old: T extends object ? Partial<T> & Pick<T, 'id' | 'iconId' | 'name' | 'type' | 'hotkey'> : T,
    new: T extends object ? Partial<T> & Pick<T, 'id' | 'iconId' | 'name' | 'type' | 'hotkey'> : T
];
export type WithIconId<T extends IBaseObject> = T & {
    iconId: string;
};
export interface IChangelogRace {
    ...
    units?: Partial<Record<keyof IRaceData['units'], ChangeTuple<WithIconId<IUnitObject>>>>;
    ...
    buildings?: Partial<Record<keyof IRaceData['buildings'], ChangeTuple<WithIconId<IUnitObject>>>>;
